...
# ── Option 1 Regression: GOOG ─────────────────────────────────────────────
# Feature columns match the regression X used in the notebook:
# cross-ticker lex sentiment scores + own sentiment_lex + sentiment_lex_lag1
MODEL_INFO = {
        "endpoint": aws_endpoint,
        "explainer": 'explainer_sentiment.shap',
        "pipeline": 'finalized_sentiment_model.tar.gz',
        # Sentiment lex scores from other tickers + own GOOG lex + lag
        "keys": ['AAPL', 'ADBE', 'AMZN', 'FB', 'JPM', 'MSFT', 'NFLX', 'TSLA', 'WMT',
                 'sentiment_lex', 'sentiment_lex_lag1'],
        "inputs": [
            # Cross-ticker sentiment (other tickers' lex score)
            *[{"name": k, "type": "number", "min": -1.0, "max": 1.0, "default": 0.0, "step": 0.01}
              for k in ['AAPL', 'ADBE', 'AMZN', 'FB', 'JPM', 'MSFT', 'NFLX', 'TSLA', 'WMT']],
            # GOOG own lex sentiment (today and yesterday)
            {"name": "sentiment_lex",      "type": "number", "min": -1.0, "max": 1.0, "default": 0.0, "step": 0.01},
            {"name": "sentiment_lex_lag1", "type": "number", "min": -1.0, "max": 1.0, "default": 0.0, "step": 0.01},
        ]
}
...
# Prediction Logic – Regression returns a float (predicted return)
def call_model_api(input_df):
    ...
    try:
        # Option 1 – Regression: model returns a continuous return value
...
# Streamlit UI
st.set_page_config(page_title="ML Deployment – GOOG Return Prediction", layout="wide")
st.title("👨‍💻 ML Deployment – GOOG Next-Day Return Prediction")
st.markdown("**Option 1 (Regression)** — Predicts the next-day simple stock return for **GOOG** "
            "using Lexicon sentiment scores.")

with st.form("pred_form"):
    st.subheader(f"Inputs – Lexicon Sentiment Scores")
    cols = st.columns(2)
    user_inputs = {}
